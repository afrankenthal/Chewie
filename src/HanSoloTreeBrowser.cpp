...
//=========================================================================
HanSoloTreeBrowser::HanSoloTreeBrowser(QWidget *parent, MainWindow * mainWindow)
    : QTreeWidget   (parent)
...
//=========================================================================
HanSoloTreeBrowser::~HanSoloTreeBrowser(void)
{
    STDLINE("Dtor",ACRed) ;
}

//=========================================================================
void HanSoloTreeBrowser::cleanDestroy(void)
{
    serviceCanvas_.clear() ;
    cSw_.clear() ;
}
...
//=========================================================================
void HanSoloTreeBrowser::clear(void)
{
    QTreeWidget::clear();
...
//=========================================================================
void HanSoloTreeBrowser::populate(TDirectory* currentDirectory)
{
...
//=========================================================================
void HanSoloTreeBrowser::populate(TFolder* currentFolder)
{
...
//=========================================================================
void HanSoloTreeBrowser::populate(TFolder* currentFolder, QTreeWidgetItem* parentWItem)
{
...
//=========================================================================
std::string HanSoloTreeBrowser::getObjectType(TObject* obj)
{
    //    STDLINE("",ACRed);
...
//=========================================================================
void HanSoloTreeBrowser::populate(TDirectory* currentDirectory, QTreeWidgetItem* parentWItem)
{
...
//=========================================================================
HanSoloTreeBrowser::dirPathDef HanSoloTreeBrowser::getFullPath(QTreeWidgetItem * wItem)
{
...
void HanSoloTreeBrowser::showContextMenu(const QPoint &)
{
    ...
    {
        serviceCanvas_[currentCanvas_] = theMainWindow_->theHanSoloFitter_->getCanvas();
                //ui_->theCanvas;
                //new CanvasWidget(theMainWindow_->theHanSoloFitter_) ;
    ...
    if( cSw_.find(currentCanvas_) == cSw_.end())
    {
        ...
        cSw_[currentCanvas_] = theMainWindow_->theHanSoloFitter_->getDrawingWindow();
                //theMainWindow_->getMdiArea()->addSubWindow(serviceCanvas_[currentCanvas_]) ;
    ...
    if( theMainWindow_->theHanSoloFitter_->plotStatBox() ){

        gStyle->SetOptStat(111111) ;
        STDLINE("plotStatBox(): " +theMainWindow_->theHanSoloFitter_->plotStatBox(), ACWhite);
    }else
        gStyle->SetOptStat(0) ;
    ...
    int nx = 1;
    int ny = 1;
    if(theMainWindow_->theHanSoloFitter_->canvasSplitType() == "X")
        nx=numberOfPlots;
    else if(theMainWindow_->theHanSoloFitter_->canvasSplitType() == "Y")
        ny=numberOfPlots;
    else if(theMainWindow_->theHanSoloFitter_->canvasSplitType() == "XY")
    {
...
//                if (currentObject_) delete currentObject_;
    ...
    cSw_[currentCanvas_]->setWindowFlags(Qt::WindowStaysOnTopHint);
    cSw_[currentCanvas_]->show() ;
    cSw_[currentCanvas_]->raise() ;
    cSw_[currentCanvas_]->update();


}

//===========================================================================
void HanSoloTreeBrowser::resizeEvent(QResizeEvent * )
{
}

//===========================================================================
void HanSoloTreeBrowser::unZoom()
{
...
//===========================================================================
void HanSoloTreeBrowser::setRange(float xMin, float xMax, float yMin, float yMax, float zMin, float zMax)
{
    if(serviceCanvas_.find(currentCanvas_) == serviceCanvas_.end()) return;
    selectedObjectsDef selectedObjects = this->getSelectedItems() ;
...
//===========================================================================
HanSoloTreeBrowser::selectedObjectsDef HanSoloTreeBrowser::getSelectedItems()
{
    //STDLINE("HanSoloTreeBrowser::getSelectedItems()",ACRed);
    HanSoloTreeBrowser::selectedObjectsDef selectedObjects ;
...
//===========================================================================
void HanSoloTreeBrowser::collectItems(QTreeWidgetItem                  * wItem,
                                HanSoloTreeBrowser::selectedObjectsDef & selectedObjects)
{
       STDLINE("HanSoloTreeBrowser::collectItems()",ACRed);
    HanSoloTreeBrowser::dirPathDef dirPath = this->getFullPath(wItem) ;
    ...
    else // Selected object resides on files
    {
        ...
        if( obj->IsA() == TFolder::Class() )
        {
            for(HanSoloTreeBrowser::dirPathDef::iterator ni=dirPath.begin()+2; ni!=dirPath.end(); ++ni)
            {
...
//===========================================================================
void HanSoloTreeBrowser::manipulateFolder( QTreeWidgetItem * wItem, int )
{
    //    STDLINE("HanSoloTreeBrowser::manipulateFolder()",ACRed);
    /*
   ...
   */
    std::cout << "manipulateFolder()" << std::endl;

    if( wItem->childCount() == 0 )
    {
        wItem->setSelected(true);
        std::cout << "HanSoloTreeBrowser::manipulateFolder() - wItem->childCount() == 0 " << std::endl;
    }
    else
    {
        this->selectAllChildren(wItem, wItem->isSelected()) ;
        std::cout << "HanSoloTreeBrowser::manipulateFolder() ||| else" << std::endl;
    }
}


//===========================================================================
//void HanSoloTreeBrowser::showHint( QTreeWidgetItem * wItem, int )
//{
...
//===========================================================================
void HanSoloTreeBrowser::selectAllChildren(QTreeWidgetItem * wItem, bool select)
{
...
//===========================================================================
void HanSoloTreeBrowser::getAllItems(QTreeWidgetItem * )
{
}

//===========================================================================
void HanSoloTreeBrowser::setCanvasSize(std::string canvasSize)
{
    std::cout << "setCanvas" << std::endl;
