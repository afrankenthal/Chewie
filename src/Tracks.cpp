        ...
        , thePlaneMapping_ (0                          )
{
    ...
    hXSlopeOutsideWindow_ = 0;
    hYSlope_              = 0;
    hYSlopeInsideWindow_  = 0;
    hYSlopeOutsideWindow_ = 0;
    hXIntercept_          = 0;
    hYIntercept_          = 0;
    hSigmaXSlope_         = 0;
    hSigmaYSlope_         = 0;
...
*/
    for(unsigned int p=0; p<hClusterHits_.size(); p++)
    {
        ...
        if(hXSlopeDistribution_[p])
        {
            delete hXSlopeDistribution_[p];
        }
        if(hYSlopeDistribution_[p])
        {
            delete hYSlopeDistribution_[p];
        }
        if(hTracksProjXY_[p])
        {
    ...
    hXSlopeDistribution_.clear();
    hYSlopeDistribution_.clear();
...
//    if(!passStandardCuts(0,data)) return;
    ...
    if (data.getColPredicted(8) > 30 && data.getIsInDetector(8))
        THREADED(hXSlopeInsideWindow_)->Fill(data.getXSlope());
    else if (data.getIsInDetector(8))
        THREADED(hXSlopeOutsideWindow_)->Fill(data.getXSlope());
    THREADED(hYSlope_    )->Fill(data.getYSlope());
    if (data.getColPredicted(8) > 30 && data.getIsInDetector(8))
        THREADED(hYSlopeInsideWindow_)->Fill(data.getYSlope());
    else if (data.getIsInDetector(8))
        THREADED(hYSlopeOutsideWindow_)->Fill(data.getYSlope());
...
void Tracks::endJob(void)
{
    ...
    ADD_THREADED(hYSlopeInsideWindow_ );
    ADD_THREADED(hYSlopeOutsideWindow_);
    ...
    hYSlopeInsideWindow_ ->GetXaxis()->SetTitle("Y Slope (rad)"         );
    hYSlopeOutsideWindow_->GetXaxis()->SetTitle("Y Slope (rad)"         );
    ...
    for (unsigned int p=0; p<thePlaneMapping_->getNumberOfPlanes(); p++)
    {
        ...
        ADD_THREADED(hXSlopeDistribution_[p]);
        hXSlopeDistribution_[p]->GetXaxis()->SetTitle("Col");
        hXSlopeDistribution_[p]->GetYaxis()->SetTitle("Row");
        ADD_THREADED(hYSlopeDistribution_[p]);
        hYSlopeDistribution_[p]->GetXaxis()->SetTitle("Col");
        hYSlopeDistribution_[p]->GetYaxis()->SetTitle("Row");
        ...
        hXSlopeDistribution_[p]->Divide(hTracksProjRC_[p]);
        hYSlopeDistribution_[p]->Divide(hTracksProjRC_[p]);
...
void Tracks::book(void)
{
    ...
    hName  = "hXSlopeInsideWindow_"            ;
    hTitle = "X slopes distribution inside the telescope window on DUT1";
    hXSlopeInsideWindow_ = NEW_THREADED(TH1F(hName.c_str(),hTitle.c_str(), 2000, -0.001, 0.001));

    hName  = "hXSlopeOutsideWindow_"            ;
    hTitle = "X slopes distribution outside the telescope window on DUT1";
    hXSlopeOutsideWindow_ = NEW_THREADED(TH1F(hName.c_str(),hTitle.c_str(), 2000, -0.001, 0.001));
    ...
    hName  = "hYSlopeInsideWindow_"            ;
    hTitle = "Y slopes distribution inside the telescope window";
    hYSlopeInsideWindow_ = NEW_THREADED(TH1F(hName.c_str(),hTitle.c_str(), 2000, -0.001, 0.001));

    hName  = "hYSlopeOutsideWindow_"            ;
    hTitle = "Y slopes distribution outside the telescope window";
    hYSlopeOutsideWindow_ = NEW_THREADED(TH1F(hName.c_str(),hTitle.c_str(), 2000, -0.001, 0.001));
    ...
    for (unsigned int p=0; p<thePlaneMapping_->getNumberOfPlanes();p++)
    {
        ...
        hName = "hXSlopeDistribution_" + planeName;
        hTitle = "Distribution of medium X slope on detector " + planeName;
        hXSlopeDistribution_.push_back(NEW_THREADED(TH2F(hName.c_str(), hTitle.c_str(), nBinsX, 0, nBinsX, nBinsY, 0, nBinsY)));

        hName = "hYSlopeDistribution_" + planeName;
        hTitle = "Distribution of medium Y slope on detector " + planeName;
        hYSlopeDistribution_.push_back(NEW_THREADED(TH2F(hName.c_str(), hTitle.c_str(), nBinsX, 0, nBinsX, nBinsY, 0, nBinsY)));
